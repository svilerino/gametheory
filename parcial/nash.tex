...
Veamos porque el grafo mencionado es un equilibrio de nash:
\begin{itemize}
...
\begin{enumerate}
	...
	\item \textbf{Una familia de grafos tal que todo equilibrio de nash usa aproximadamente una fraccion $\frac{n}k{}, k \in \mathbb{N}$, asumimos que si el grafo es conexo, no es union de grafos disjuntos.(pues hay una arista que los une, entonces no son disjuntos)}. Vemos que es una generalizacion del item anterior, donde k=2. Notemos que la fraccion $\frac{n}{k}$ es \textbf{aproximada}. 




	Esta familia se compone con k nubes de conjuntos independientes de nodos.
\end{enumerate}
